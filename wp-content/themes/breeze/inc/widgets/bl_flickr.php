...
class blu_flickr_widget extends WP_Widget {
	function blu_flickr_widget() {
		$widget_ops = array( 'classname' => 'blu_flickr', 'description' => __( 'Displays recent Flickr photos in a widget', 'bluth_admin' ) );

		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'flickr-photos' );

		$this->WP_Widget( 'flickr-photos', __( 'Bluth Flickr', 'bluth_admin' ), $widget_ops, $control_ops );
	}
				      	...
				      	<?php
							...
							if ( $rsp_obj['stat'] == 'ok' ) {
								...
								foreach ( $rsp_obj['photos']['photo'] as $photo ) { ?>
										<div class="swiper-slide"> 
											<a style="background-image: url('http://farm<?php echo $photo['farm'] ?>.staticflickr.com/<?php echo $photo['server'] . '/' . $photo['id'] . '_' . $photo['secret']; ?>.jpg');"></a>
										</div><?php
									}
								}
