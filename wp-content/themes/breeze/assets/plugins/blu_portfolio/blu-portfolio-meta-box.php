<?php
	...
	function blu_portfolio_options( $post )  { 
	    ...
	    <fieldset class="full">
	        <input type="text" id="blu_portfolio_link" class="full large" name="blu_portfolio_link" placeholder="Link to project.." value="<?php echo $blu_portfolio_link; ?>" style="margin: 10px 0; width:100%; font-size: 16px;" />  
	    </fieldsedt>
	    <fieldset class="full">
	        <input type="text" id="blu_portfolio_client" class="full large" name="blu_portfolio_client" placeholder="Client Name.." value="<?php echo $blu_portfolio_client; ?>" style="margin: 10px 0; width:100%; font-size: 16px;" />  
	    </fieldsedt>
	    <fieldset class="full">
	        <input type="text" id="blu_portfolio_client_link" class="full large" name="blu_portfolio_client_link" placeholder="Client Link.." value="<?php echo $blu_portfolio_client_link; ?>" style="margin: 10px 0; width:100%; font-size: 16px;" />  
	    </fieldsedt>
	    <fieldset class="full">
		    <h4>Gallery Images: <small>Add images to display in the slider</small> <a href="javascript:void(0);" class="bl_add_field">+ Add image</a></h4><?php
	    ...
	    <fieldset class="full array_fieldset">
		    <h4>List Fields: <small>Add fields to display in the items description</small><a href="javascript:void(0);" class="bl_add_field_complex">+ Add field</a></h4><?php
		    foreach( $blu_portfolio_field as $key => $parent ){ 
		    	foreach( $parent as $array_item_key => $array_item ){ ?>
				    <div class="array_item" data-key="<?php echo $array_item_key; ?>" data-name="blu_portfolio_field">
	        			<input type="text" id="blu_portfolio_field[<?php echo $array_item_key; ?>][title]" class="blu_item_title full large" name="blu_portfolio_field[<?php echo $array_item_key; ?>][title]" placeholder="Name of list" value="<?php echo $array_item['title']; ?>" />  
		    			 <?php
		    			foreach( $array_item['item'] as $input_item_key => $input_item ){ ?>
		    				<div class="input_item_group">
		    					...
		    					<input placeholder="Item.." name="blu_portfolio_field[<?php echo $array_item_key; ?>][item][<?php echo $input_item_key; ?>]" value="<?php echo $input_item; ?>" data-key="<?php echo $input_item_key; ?>"  data-arrayname="item0<?php echo $array_item_key; ?>" class="blu_item full" type="text" >
		    				</div>
		    				<?php
		    			} ?>
			    		<a href="javascript:void(0);" class="bl_add_field_complex_child">+ Add field</a>
			    		<button type="button" class="close">&times;</button>
				    </div><?php
				}
			} ?>
